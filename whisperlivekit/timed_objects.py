...
class Transcript(TimedText):
    ...
    @classmethod
    def from_tokens(
        cls, tokens: List[ASRToken], sep: Optional[str] = None, offset: float = 0
    ) -> "Transcript":
...
class Segment(TimedText):
    ...
    @classmethod
    def from_tokens(
        cls, tokens: List[Union[ASRToken, Silence]], is_silence: bool = False
    ) -> Optional["Segment"]:
        ...
        if is_silence:
            return cls(
                start=start_token.start, end=end_token.end, text=None, speaker=-2
            )
        else:
            return cls(
...
class FrontData:
    ...
    speaker_ids: Dict[int, str] = field(default_factory=dict)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.speaker_ids:
            _dict["speaker_ids"] = self.speaker_ids
        if self.error:
...
class State:
    ...
    new_tokens_buffer: List[Any] = field(
        default_factory=list
    )  # only when local agreement
